...
non_info_fcn <- function(indx, mat, prob, meta, pseudo_bulk, niter = 100){
  ...
  ind_data <- matrix(NA,nrow = length(subject), ncol = 7)
  for(j in 1:length(subject)){
    
    cell<-which(meta$Donor==subject[j])
    s<-which(sub$Donor==subject[j])
    
    ind_data[j,1]<- as.numeric(sub$pheno[s]=="IPF")    
    ind_data[j,2]<- length(which(meta$Donor==subject[j]))
    ind_data[j,3]<- prob[which(prob$Donor==subject[j]),2]
    ind_data[j,4]<- -log(ind_data[j,3])
    ind_data[j,5]<- pseudo_bulk[indx, which(colnames(pseudo_bulk)==subject[j])]
    ind_data[j,6]<- max(count[cell]) 
    ind_data[j,7]<- ind_data[j,1] * ind_data[j,4]
  }
  
  rownames(ind_data)<-subject
  colnames(ind_data)<-c("Pheno","Num_Cells","Prob_mean","Negative_log","Mean","Maximum","Interaction")

  ### generate EST in Noninformative prior.R
  pheno <- ind_data[,1]  #1 IPF, 0 control
  nobs1 <- ind_data[,2]  # individual number of cells including all cells
  prob_mean <- ind_data[,3]  # Probability mean
  log_prob_mean <- -log(prob_mean)  # negative log scale mean
  tgfbl_mean1 <- ind_data[,5]  # Mean of the tgfb1
  tgfbl_max1 <- ind_data[,6]  # Maximum of tgfb1
  log_inter <- pheno*log_prob_mean  # Interaction
  
  # design matrix from ind_data
  Y <- tgfbl_mean1
  X <- matrix(c(rep(1, length(Y)), pheno, log_prob_mean),ncol=3)
  ...
  saveRDS(list(EST = EST, STD = STD, pval = p_value), paste0("ninfo_output_", gname, ".rds"))
